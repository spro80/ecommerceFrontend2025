...
export async function getProducts() {
  try {
    ...
    const { enhanceProductsWithImage } = await import('./images.js');
    return enhanceProductsWithImage(data);
  } catch (error) {
    if (shouldFallbackToMock(error)) {
      const { products } = await import('../data/products.js');
      const { enhanceProductsWithImage } = await import('./images.js');
      return enhanceProductsWithImage(products);
    }
    throw error;
  }
}
...
export async function getProductByIdApi(id) {
  try {
    ...
    const { enhanceProductWithImage } = await import('./images.js');
    return enhanceProductWithImage(data);
  } catch (error) {
    if (shouldFallbackToMock(error)) {
      const { getProductById } = await import('../data/products.js');
      const product = getProductById(id);
      const { enhanceProductWithImage } = await import('./images.js');
      return enhanceProductWithImage(product);
    }
    throw error;
  }
}
